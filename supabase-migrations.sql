-- ============================================
-- PRAYER MARATHON APP - COMPLETE DATABASE SETUP
-- ============================================
...
-- Table: day_resources (Ressources journalières)
...
-- Table: testimonials (if not exists)
...
-- Table: prayer_requests (if not exists)
...
-- Create index for prayer requests
...
-- Table: days (Programme journalier - if not exists)
...
CREATE POLICY "Admins can insert day resources"
    ...
    WITH CHECK (true); -- You can add admin check here
...
CREATE POLICY "Admins can update day resources"
    ...
    USING (true); -- You can add admin check here
...
--
-- 1. Create bucket: 'day-resources'
--    - Public: true
--    - File size limit: 50MB
--    - Allowed MIME types: image/*, video/*, audio/*, application/pdf
--
-- 2. Create bucket: 'testimonial-photos'
--    - Public: true
...
-- - SELECT: Allow public access
-- - INSERT: Allow authenticated users (own files)
-- - UPDATE: Allow authenticated users (own files)
